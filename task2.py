...
def top_bottom_images(model, test_loader, device, class_idx, n=5):
    model.eval()
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            images, labels = images.to(device), labels.to(device)
            outputs = model(images)
            score = torch.softmax(outputs, dim=1)[:, class_idx]

            for img, score in zip(images, score):
                class_scores.append(img.cpu(), score.item())

    class_scores.sort(key = lambda x: x[1], reverse = True)
    return class_scores[:n], class_scores[-n:]
